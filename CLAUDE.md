...
**Diet AirDS** is a multi-team design system supporting 10 sports organizations. It lives at `https://diet-air-ds.vercel.app` and provides a token-based CSS foundation + a React/TypeScript component library for building UI components across different team brands with both light and dark mode theming.

The CSS layer is intentionally lean: pure CSS tokens + utility classes, no framework dependencies. The system also ships 13 typed React components (`src/`) built on top of those same tokens. Components are built by composing existing tokens and classes — never by writing custom CSS from scratch.
...
- **React component library** — 13 typed components built in `src/`, deployed at `https://diet-air-ds.vercel.app`. Storybook at `localhost:6006` via `npm run dev`.
- **React site documentation** — Documentation pages live at `/documentation/*` routes in the React app (see `src/pages/docs/`).
...
├── *.html                    # Static demo and walkthrough pages
...
├── app.html                  # React app entry point (Vite template → becomes site-dist/index.html)
├── vite.config.ts            # Library mode build config (outputs to dist/)
├── vite.site.config.ts       # Site build config (outputs to site-dist/)
├── src/                      # React/TypeScript source
│   ├── components/           # 13 typed components (Button, Card, ListRow, Input, etc.)
│   ├── pages/                # Site pages (HomePage, docs/*, CheckoutPage, etc.)
│   ├── layouts/              # SiteShell, DocShell, OrgNav
│   ├── context/              # ThemeContext (manages data-theme/data-mode on <html>)
│   └── hooks/                # useSelectSync
├── site-dist/                # Built React site — output of npm run build:site (deployed to Vercel)
├── storybook-static/         # Built Storybook — output of npm run build-storybook
├── dist/                     # Built component library — output of npm run build (ES + CJS)
...
├── scripts/                  # Build + Sanity sync scripts (Node.js ESM)
...
**Static HTML files** — open any `.html` file directly in a browser to preview CSS components.

**React site dev server** — `npm run site` starts the Vite dev server for the React app.

**Storybook** — `npm run dev` starts Storybook at `localhost:6006` for component development.
...
### Deploy workflow
Edit local files → `git push origin main` → Vercel runs `npm run build:site` → builds React app to `site-dist/` → deploys. CSS and React changes go live together.
...
| File | Purpose |
|---|---|
| `agent-guide.md` | Complete build reference for AI agents — CSS rules, React component API, patterns, examples, master prompt template |
| `react-migration.md` | React migration notes, component status, and architecture details |
...
| `src/index.ts` | React library entry — exports all 13 components |
